...
import { intermediateNextJsQuestions } from "./intermediateQuestion";

export default function NextJsQuiz() {
  const [isAnswerVisible, setIsAnswerVisible] = useState<boolean[]>(new Array(intermediateNextJsQuestions.length).fill(false));
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto bg-gradient-to-r from-indigo-200 to-purple-300 rounded-xl shadow-lg">
      <h1 className="text-5xl font-extrabold text-center text-gray-900 mb-8 tracking-wide">
        Next.js Quiz - Intermediate Level
      </h1>
      <div className="space-y-8">
        {intermediateNextJsQuestions.map((question, index) => (
          <div
            key={index}
